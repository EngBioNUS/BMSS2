...
def load_model(filestream):
    ...
    for line in filestream1:
        if len(line.rstrip()) == 0:
            ...
            line1  = split_line(line)
            content['variables'].append(line1)
        elif content_type == 'equations' or content_type == 'ODEs':
            line1  = split_line(line)
            content['equations'].extend(line1)    
        elif content_type == 'decomp':
            line1 = line.rstrip()
            if content['decomp']:
                content['decomp'] += line1
            else:
                content['decomp'] = line1

        #Aliases for params and outputs    
        elif content_type == 'unknown params' or content_type == 'unknown parameters' or content_type == 'parameters':
            line1 = split_line(line)
            content['params'].extend(line1)
        elif content_type == 'measured outputs':
            line1 = split_line(line)
            content['outputs'].extend(line1)
        else:
            line1  = split_line(line)
...
#Export as .py
###############################################################################    
def write_to_file(variables, equations, measured_outputs, states, parameters, input_conditions, ics, decomposition=[], known_params={}, filename=''):
    text     = '\n\n'.join([export_model(variables, equations),
                            export_args(measured_outputs, states, parameters, input_conditions, ics, decomposition, known_params),
                            export_run()])
    if len(filename) > 0:
...
def export_args(measured_outputs, states, unknown_parameters, input_conditions, ics, decomposition, known_params):
    ...
    result += make_matrix('measured_states   ', measured_outputs)
...
if __name__ == '__main__':
    ...
    filename = 'balance_model'
    data     = import_txt(filename)
    
    write_to_file(data, filename='balance_model.py', ics_values={}, outputs=[], states=[], params=[], decomp=[])

    
    d = importlib.import_module(filename)
    d.runIA()
    
##    filename = 'ConstDouble'
##    data     = import_txt(filename)
##    results = permutational_IA(data)

##    #For testing
##    #Use this to write model into python file
##    #If default values are used, the function will refer to the values specified in the original txt file
##    write_to_file(data, filename='dummy.py', ics_values={}, outputs=[], states=[], params=[], decomp=[])
