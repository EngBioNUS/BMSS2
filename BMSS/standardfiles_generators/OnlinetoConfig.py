...
def get_online_biomodel(Biomodels_ID):
...
def sbmltoconfig(sbmlstr, system_type, tspan, Model_name):
    ...
    Config_filepath = Path.cwd().joinpath(str(Model_name) + '.ini')
    settings_filepath = Path.cwd().joinpath('sim_settings_template_' + str(Model_name) + '.ini')
...
def clean_eqns(store_species):
    ...
    to format in config file
    :param store_species: ID of species in reaction
    :returns: list format if reactions have more than 1 variable in reaction
    :returns: string format if reactions only have 1 variable in reaction
    '''
...
def get_posreaction(reaction_formula_store, value, reaction_reactant_end, reaction_product_end, reactioneqns):
    ...
    tempstore_product_list = clean_eqns(tempstore_product)
...
def get_negreaction(reaction_formula_store, value, reaction_reactant_start, reaction_reactant_end, reactioneqns):
    ...
    tempstore_reactant_list = clean_eqns(tempstore_reactant)
