...
def database_to_sbml():
    ...
    model_number = int(input("Enter only the number from the list of the models you would like to convert to SBML: ")) 
    model_chosen = lst[model_number]
    
    print(model_chosen)
    
    sindex = []
    for count, v in enumerate(lst_setting):
        if model_chosen in v[0]:
            sindex.append(count)
            
    if not sindex:
        print('No default setting is available for this model.')
    else:
        print('List of Settings')
        
        for i, s in enumerate(sindex):
            print('[',i, ']', lst_setting[s])
            
        setting_number = int(input("Enter only the number from the list of the model settings you would like to convert to SBML: ")) 
        
        setting_number_zero = lst_setting[sindex[setting_number]][0]
        setting_number_one = lst_setting[sindex[setting_number]][1]
