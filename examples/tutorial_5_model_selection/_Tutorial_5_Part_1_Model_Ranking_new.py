...
- Calculate ic from fitted parameters
...
if __name__ == '__main__':
    ...
    '''
    In order to calculate the ic, we need the data, models, priors and
    parameters for evaluation. 
    '''
    table = il.calculate_ic(data   = sampler_args['data'], 
                            models = sampler_args['models'], 
                            priors = sampler_args['priors'],
                            params = accepted.iloc[-10:]
                            )
    
    print('ic of each row of params for each model.')
    print(table)
    print()
    
    '''
    rank_ic accepts a DataFrame containing ic values indexed under 
    ic_column_name. It then sorts the DataFrame and adds columns for the normalized
    ic and the evidence for that model. The original columns in the input
    DataFrame remain untouched.
    
    As can be seen from the result, rank_ic isn't picky about what indices and 
    columns you use as long as your DataFrame has its ic values indexed under ic_column_name.
    '''
    
    ranked_table  = il.rank_ic(table, ic_column_name='ic Value', inplace=False)
