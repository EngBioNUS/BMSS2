...
'''
Example 4: This is to check the predictive performance of Model 4 from
the 2nd iteration against the three synthetic data sets.
'''
...
def read_data(states_and_filenames, n_models=1):
    ...
    for state, filename in states_and_filenames.items():
        ...
        for model_num in range(1, n_models+1):
            ...
            #init.setdefault(model_num, {})[state] = {}
...
if __name__ == '__main__':
    ...
    #Import synthetic data for the two inducers to be used for comparing predictions
    test_data_file1 = Path.cwd() /'data'/'Ind_synthetic_data.csv'
    test_data_Ind = pd.read_csv(test_data_file1).to_numpy()
    
    test_data_file2 = Path.cwd() /'data'/'Inde_synthetic_data.csv'
    test_data_Inde = pd.read_csv(test_data_file2).to_numpy()
    
    #Read data (refer to the subfunction above)
    ...
    for i in range(5):
        ...
        labels = {}
        for model_num in sampler_args['models']:
            plot_index[model_num]  = ['Fluor/OD', 'Ind', 'Inde']
            titles[model_num]      = {'Fluor/OD': 'Pep Model '+ str(model_num),
                                      'Ind': 'Ind Model '+ str(model_num),
                                      'Inde': 'Inde Model '+ str(model_num)}
        ...
        # plot the synthetic data for the inducers for comparison
        AX[1]['Ind'].plot(test_data_Ind[:,0], test_data_Ind[:,1:5], 'x', markersize = 6,label='Test Data')
        AX[1]['Ind'].legend(loc='upper right', fontsize=14)
        
        AX[1]['Inde'].plot(test_data_Inde[:,0], test_data_Inde[:,1:5], 'x', markersize = 6,label='Test Data')
        AX[1]['Inde'].legend(loc='upper right', fontsize=14)
        ...
        '''predict deg_Pep when increasing deg_Pep by 1.5x''' 
